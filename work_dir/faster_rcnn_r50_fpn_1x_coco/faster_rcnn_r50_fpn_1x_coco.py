...
train_pipeline = [
    ...
    dict(
        type='Normalize',
        mean=[94.46, 100.35, 94.38],
        std=[69.13, 72.75, 70.68],
        to_rgb=True),
...
data = dict(
    ...
    train=dict(
        type='JRDBDataset',
        ann_file='data/JRDB/JRDBtrain/train.txt',
        img_prefix='data/JRDB/JRDBtrain/image_2',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='LoadAnnotations', with_bbox=True),
            dict(type='Resize', img_scale=(3760, 480), keep_ratio=True),
    ...
    val=dict(
        type='JRDBDataset',
        ann_file='data/JRDB/JRDBval/val.txt',
        img_prefix='data/JRDB/JRDBval/image_2',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(
                type='MultiScaleFlipAug',
    ...
    test=dict(
        type='JRDBDataset',
        ann_file=
        'data/JRDB/JRDBtotal_with_preprocessing_except_fullyoccluded/total.txt',
        img_prefix=
        'data/JRDB/JRDBtotal_with_preprocessing_except_fullyoccluded/image_2',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(
                type='MultiScaleFlipAug',
